...
def loadData(path,k=100):
    ...
    train_set_y =np.asarray(datasets[0][:,-1],"int64")
...
def normalize_text(text):
    ...
    for seg in text   :
        if seg not in string.punctuation:
        # if seg not in stops and seg not in string.punctuation:
            resulutStr +=seg +' '
    return resulutStr

def remove_stops_puncatuation(filePath,outfilePath):
...
if __name__=="__main__":
    ...
    run('Json/AlltraininwithPosition.json',160)